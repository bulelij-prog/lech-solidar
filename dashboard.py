...
# --- Helper : extraire le texte de réponse (nouveau format) ---
def extraire_texte_reponse(data):
        """Extrait le texte de réponse depuis le nouveau format de la Cloud Function.
            Nouveau format : fulfillment.messages[0].text
                Ancien format (fallback) : fulfillmentMessages[0].text.text[0]
                    """
    # Nouveau format : fulfillment -> messages -> [0] -> text
    ...
    if fulfillment:
                ...
                if messages and isinstance(messages, list):
                        ...
                        # Fallback ancien format : fulfillmentMessages -> [0] -> text -> text -> [0]
                        old_messages = data.get("fulfillmentMessages", [])
    if old_messages:
                return old_messages[0].get("text", {}).get("text", ["Pas de réponse"])[0]

    return "Pas de réponse reçue du moteur."

# --- Helper : formater réponse avec alertes ---
def afficher_reponse(data):
        reply = extraire_texte_reponse(data)
    if reply == "Pas de réponse reçue du moteur.":
                st.warning(reply)
        return
                ...
                if audit_text.strip():
                                with st.spinner("🔍 Analyse de conformité en cours (CCT CP 330 + Protocoles Brugmann + Législation)..."):
                                                    ...
                                                    if data:
                                                                            ...
                                                                            reply = extraire_texte_reponse(data)
                                                                            if reply and reply != "Pas de réponse reçue du moteur.":
